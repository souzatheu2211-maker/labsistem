...
Looking at the previous response, it ended abruptly inside a `dyad-write` block for `src/pages/Results.tsx`.

The last line was:
`if (`

I need to complete the `handleSaveResult` function, the rest of the `Results` component, and then provide the SQL command and the `Reports.tsx` update as planned in the previous turn.

Wait, I should check the context of `src/pages/Results.tsx` from the previous turn's `dyad-write`.
...
Wait, I need to check where it ended.
`if (`

Ah, it was in `handleSaveResult`.
...
Then I need to update `Reports.tsx` to handle `____`.

And then the SQL.

Let's go.allFinished) {
        await supabase.from('services').update({ status: 'finalizado' }).eq('id', selectedService.id);
      }
